...
result FacebookForm::OnInitializing(void) {
	...
	__pLbl = new Label();
	__pLbl->Construct(GetBounds(), L"");

	__accessToken.ExpiryTime = -1;
	__accessToken.AccessToken = L"";
		...
		if (__accessToken.ExpiryTime > 0) {
			...
			__pLbl->SetText(
					L"Access Token:\n" + __accessToken.AccessToken
							+ L"\n\nExpires:\n"
							+ Long::ToString(__accessToken.ExpiryTime));
			AddControl(__pLbl);
			RequestRedraw(true);
			r = CreateFacebookAlbum();
			if (r != E_SUCCESS)
				AppLogException(
						"Error creating facebook album, problem with http transaction: ", GetErrorMessage(r));
		}
	} else {
...
void FacebookForm::OnActionPerformed(const Tizen::Ui::Control& source,
		int actionId) {
	switch (actionId) {
	default:
		break;
